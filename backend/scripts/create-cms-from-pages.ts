...
import { PrismaClient, CMSStatus } from '@prisma/client';
...
// Contact Us content (converted from JSX to HTML)
const contactContent = `<p class="text-gray-600 mb-8">Have questions about LangkahKecil, need help with your account, or want to share feedback? We&apos;d love to hear from you.</p>
...
// Privacy Policy content (from frontend/src/app/cms/privacy/page.tsx)
...
// Terms of Service content (from frontend/src/app/cms/terms/page.tsx)
...
async function main() {
  console.log('🌱 Creating CMS content from page defaults...\n');

  const contents = [
    {
      title: 'Contact Us',
      slug: 'contact',
      content_html: contactContent.trim(),
      status: 'published' as CMSStatus,
    },
    {
      title: 'Privacy Policy',
      slug: 'privacy',
      content_html: privacyContent.trim(),
      status: 'published' as CMSStatus,
    },
    {
      title: 'Terms of Service',
      slug: 'terms',
      content_html: termsContent.trim(),
      status: 'published' as CMSStatus,
    },
  ];
  ...
  for (const content of contents) {
    try {
      ...
      if (existing) {
        ...
        console.log(`✓ Updated CMS content: ${content.slug} (${content.title})`);
      } else {
        // Create new content
        await prisma.cMSContent.create({
          data: content,
        });
        console.log(`✓ Created CMS content: ${content.slug} (${content.title})`);
      }
    } catch (error) {
  ...
  console.log('\n✅ CMS content creation completed!');
}
