...
type IOFieldType = 'fs_1_to_5' | 'percent_0_100' | 'number_minutes' | 'text';
    ...
    sections: [
      {
        ...
        fields: [
          ...
          { key: 'other_major_1_label', label: { en: 'Other Major Disruptive Behavior (Specify) 1', id: 'Perilaku mengganggu lain (isi) 1' }, type: 'text', required: true },
          { key: 'other_major_1', label: { en: 'Other Major Disruptive Behavior 1 (F / S)', id: 'Perilaku mengganggu lain 1 (F / S)' }, type: 'fs_1_to_5', required: true },
          { key: 'other_major_2_label', label: { en: 'Other Major Disruptive Behavior (Specify) 2', id: 'Perilaku mengganggu lain (isi) 2' }, type: 'text', required: true },
          { key: 'other_major_2', label: { en: 'Other Major Disruptive Behavior 2 (F / S)', id: 'Perilaku mengganggu lain 2 (F / S)' }, type: 'fs_1_to_5', required: true },
      ...
      },
      {
        ...
        fields: [
          { key: 'attention_span_minutes', label: { en: 'Attention Span (Average Duration, minutes)', id: 'Rentang perhatian (rata-rata, menit)' }, type: 'number_minutes', required: true },
          { key: 'eye_contact_on_request_pct', label: { en: 'Eye-to-Face-Contact: looking on request (%)', id: 'Kontak mata: menatap saat diminta (%)' }, type: 'percent_0_100', required: true },
          { key: 'eye_contact_name_called_pct', label: { en: 'Eye-to-Face-Contact: name is called (%)', id: 'Kontak mata: saat dipanggil namanya (%)' }, type: 'percent_0_100', required: true },
          { key: 'eye_contact_talking_listening_pct', label: { en: 'Eye-to-Face-Contact: talking/listening (%)', id: 'Kontak mata: saat berbicara/mendengarkan (%)' }, type: 'percent_0_100', required: true },
          { key: 'looking_at_task_materials_pct', label: { en: 'Looking at Task Materials (%)', id: 'Melihat bahan/tugas (%)' }, type: 'percent_0_100', required: true },
          { key: 'follows_simple_directives_with_gestures_pct', label: { en: 'Follows Simple Directives with Gestures (%)', id: 'Mengikuti instruksi sederhana dengan isyarat (%)' }, type: 'percent_0_100', required: true },
        ],
      },
      {
        key: 'compliance',
...
function ensureTemplateShape(json: any): IOTemplateJson {
  const version = Number(json?.version || 1);
  const sections: IOSection[] = Array.isArray(json?.sections) ? json.sections : [];
  return {
    version,
    sections: sections.map((s: any) => ({
      key: String(s?.key || ''),
      title: { en: String(s?.title?.en || ''), id: String(s?.title?.id || '') },
